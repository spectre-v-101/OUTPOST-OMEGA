...
bg_music = Audio('assets/beyond-the-dark-battle-epic-orchestral-274992.mp3', loop=True, autoplay=True,volume=1.5)
...
boss = Entity(
   ...
   # color=color.black33,
    scale=1.7,
    position=(0,0, 50),   # Starts far in Z
    #enabled=False
    collider='box'
)
obstacles.append(boss)
Boss_text=Text(text='Command Ship', color=color.violet,position=(0,2.5), origin=(0, 0), scale=(20,20),parent=boss)    
Boss_health_bar=Entity(model='quad', color=color.red,parent=boss, scale=(7,0.3), position=(0,2),)
...
def enemy_shoot(enemy):
    ...
    direction = (spaceship.world_position - enemy.world_position).normalized()
    bullet = Entity(
        model='Sphere',
        color=color.red,
        scale=(0.1,0.1,1.2),
        position= enemy.position+direction*1.5,  # towards player
        collider='box'
    )
    bullet2=Entity(
        model='Sphere',
        color=color.red,
        scale=(0.1,0.1,1.2),
        position= enemy.position+direction*1.5+Vec3(1,0,0),  # towards player
        collider='box'
    )
    bullet3=Entity(
        model='Sphere',
        color=color.red,
        scale=(0.1,0.1,1.2),
        position= enemy.position+direction*1.5+Vec3(2,0,0),  # towards player
        collider='box'
    )
    bullet4=Entity(
        model='Sphere',
    ...
    bullet4.direction=direction
    bullet3.direction=direction
    bullet2.direction=direction
    bullet.direction = direction # enemy shoots backward (toward player)
    enemy_bullets.append(bullet)
    enemy_bullets.append(bullet2)
    enemy_bullets.append(bullet3)
    enemy_bullets.append(bullet4)
...
def update_health_bar_boss():
    # Scale based on current health
    global boss_health
    Boss_health_bar.scale_x = 7 * (boss_health / 100)
...
objective_goal = 50
enemies_destroyed = 0
objective_text = Text(text='OBJECTIVE: Destroy the Alien Command ship', position=(-0.78, 0.48), scale=1.3, color=color.cyan, parent=camera.ui)
...
def update():
   ...
   # decal.rotation=(decal.rotation.x, decal.rotation.y, decal.current_roll)
    ...
    boss.y = 4+math.sin(time.time() * 2) * 2
   # else:
       # boss.y = 4-math.sin(time.time() * 2) * 2
    if( random.random()<=0.5):
        boss.x=4+math.sin(time.time() * 2) * 2
    #else:
       # boss.x=4-math.sin(time.time() * 2) * 2
    #elif(boss.x==4 and boss.y==4 and random.random()>0.5):
        #boss.y=4+math.sin(time.time() * 2) * 2
    ...
    for enemy in obstacles:
        if random.random() < 0.25:  # 1% chance per frame
             enemy_shoot(enemy)
    ...
    for bullet in bullets[:]:
        ...
        for obstacle in obstacles[:]:
            if bullet in bullets and bullet.intersects(obstacle).hit:
                ...
                bullets.remove(bullet)
                boss_health-=0.7
                update_health_bar_boss()
                explosion_sound.play()
                #enemies_destroyed += 1
                #objective_text.text = f'Destroy the alien command ship}'
